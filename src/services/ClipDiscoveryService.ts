...
export class ClipDiscoveryService {
    ...
    this.socket.on("message", (msg, rinfo) => {
      try {
        ...
        if (message.includes('"service":"CLIP"')) {
          const data: Announcement = JSON.parse(msg.toString());
  ...
  async setDiscoverable(discoverable: boolean) {
  ...
  handleServerAnnouncement(data: Announcement, rinfo: dgram.RemoteInfo) {
